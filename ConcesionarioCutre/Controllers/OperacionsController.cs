...
namespace ConcesionarioCutre.Controllers
{
    public class OperacionsController : Controller
    {
        ...
        {
            //List<String> listaModeloMarcas = new List<string>();

            ViewBag.MODELOMARCA = db.Coches.Select(c => new SelectListItem { Value =c.ID.ToString() , Text= c.MARCA.ToString() + " " + c.MODELO.ToString() });
